...
# ==============================================================================
# run_all_checkpoints2.sh — GRPO の全 global_step checkpoint を順番に推論・評価
# Usage:
#   bash run_all_checkpoints2.sh                          # デフォルト MODEL_DIR, INFER_MODE=all
#   bash run_all_checkpoints2.sh <MODEL_DIR>              # INFER_MODE=all
#   bash run_all_checkpoints2.sh <MODEL_DIR> generate     # 通常の自由生成のみ
#   bash run_all_checkpoints2.sh <MODEL_DIR> constrained  # 語彙制約デコーディング (+閾値校正)
#   bash run_all_checkpoints2.sh <MODEL_DIR> all          # 上記すべて
#
# 期待するディレクトリ構造:
#   <MODEL_DIR>/
#     global_step_50/
...
restore_config() {
    if [ -f "$CONFIG_BAK" ]; then
        ...
        rm -f "$CONFIG_BAK"
        echo "config2.yaml を元の状態に復元しました。"
    fi
}
trap restore_config EXIT

# config2.yaml をバックアップ
cp "$CONFIG" "$CONFIG_BAK"
echo "config2.yaml をバックアップしました: $CONFIG_BAK"

# 推論+評価用に config を書き換える (dev/test のパスは data セクションから取得)
write_infer_config() {
    local model_path="$1" split="$2" decoding="$3" pred_file="$4" result_file="$5"
    uv run python - "$CONFIG_BAK" "$CONFIG" "$model_path" "$split" "$decoding" "$pred_file" "$result_file" <<'PYEOF'
import sys
import yaml

bak, cfg_path, model_path, split, decoding, pred_file, result_file = sys.argv[1:8]
with open(bak) as f:
...
print(f"config 更新: split={split} decoding={decoding}")
...
echo "=============================="
echo " Batch Evaluation: All GRPO Checkpoints (dev + test, mode=$INFER_MODE)"
